...
- 这是一个面向 Trae 的 Skill，优先使用通用文件读写与 shell 能力，不依赖 Claude Code 专属命令
- 涉及预览时，按当前操作系统选择可用命令：macOS 用 `open`，Linux 用 `xdg-open`，Windows 用 `start` 或 `Start-Process`
- 如果当前环境不允许自动打开浏览器，就正常生成 HTML 文件，并明确告知用户输出路径
- 输出目录默认使用 skill 根目录下的 `output/`
...
3. 在当前系统中选择可用方式打开浏览器预览：
   - macOS：`open`
   - Linux：`xdg-open`
   - Windows：`start` 或 `Start-Process`
